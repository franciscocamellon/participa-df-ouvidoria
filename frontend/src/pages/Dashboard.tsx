...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    const byStatus = occurrenceStatuses.reduce((acc, status) => {
      acc[status.id] = occurrences.filter(o => o.status === status.id).length;
      return acc;
    }, {} as Record<string, number>);
    
    const byCategory = occurrenceCategories.reduce((acc, cat) => {
      acc[cat.id] = occurrences.filter(o => o.category === cat.id).length;
      return acc;
    }, {} as Record<string, number>);

    const byUrgency = urgencyLevels.reduce((acc, level) => {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-muted-foreground">
                Visão geral de todas as ocorrências registradas na plataforma
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            ...
            <Card className="border-border">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {stats.resolved} ocorrências concluídas
                </p>
              </CardContent>
            </Card>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-foreground mb-1">
                  Métricas de Desempenho do Piloto
                </p>
                <p className="text-xs text-muted-foreground">
                  Tempo médio de triagem: 4.2h • Taxa de encaminhamento correto: 94% • 
                  Taxa de retorno em 7 dias: 87% • Dados anonimizados e agregados.
                </p>
              </div>
            </div>
          </div>
